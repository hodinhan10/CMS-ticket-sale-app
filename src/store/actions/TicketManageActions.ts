...
import { GET_CHECKING_GROUP, GET_CHECKING_GROUP_DETAILS, TicketAction } from '../types';
...
export const detailsTicketEvent = ( id : any): ThunkAction<void, RootState, null, TicketAction> => {
  // console.log(id);
  return async dispatch => {
    try {
      db.collection("TicketManage").doc(id)
        .get().then((doc) => {
          let arr1: any = [];
          arr1.push({ ...doc.data(), ...{ id: doc.id } })
...
export const UpdateDayEndTicketEvent = (DateEnd:any, id : any): ThunkAction<void, RootState, null, TicketAction> => {
